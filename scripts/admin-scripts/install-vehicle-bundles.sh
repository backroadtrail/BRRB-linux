...
# OLSRD
if ! is_bundle_installed "network.mesh_olsrd"; then
    ../configure.sh network mesh-olsrd install
elif ! is_bundle_current "network.mesh_olsrd"; then
    ../configure.sh network.mesh-olsrd upgrade
fi

#ACCESS-POINT
if ! is_bundle_installed "network.access_point"; then
    ../configure.sh network access-point install
elif ! is_bundle_current "network.access_point"; then
    ../configure.sh network access-point upgrade
fi
